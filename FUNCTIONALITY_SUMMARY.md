...
```js
{
  ...
  notModeled: [...],   // Session 81: full-string entries; 'below-grade-wall'/'slab-on-grade' SHED when ground resolves; infiltration(partial)/ventilation/duct-pipe/floor-over-unheated remain; 'solar-gain' REMOVED (#139)
}
```

**Extensible spine:** Adding a genuine HEATING loss endpoint = a bucket + loop body (in-engine) or a separate derive-fn wired into the output (like §11c ground-coupled); no refactor. below-grade-wall and slab-on-grade were exactly such additions; infiltration (Session 81, §11e) is another. `notModeled[]` marks the current gap explicitly and shrinks as heating endpoints land. NOTE: `'solar-gain'` is NOT a heating bucket — F280 credits zero solar against heating; solar is a COOLING-only term for a future `deriveF280Cooling` endpoint (see ADDITIONAL_FUNCTIONALITY.md #130). **It was REMOVED from the heating `notModeled[]` entirely in Session 81 (#139)** — a cooling-only term does not belong in a list of HEATING gaps.
...
**⚠ Session-80 F280 conformance-audit note (docs-only; describes a gap in the CURRENT code, no code changed this session):** the read-only heating-path audit against the licensed CSA F280:12 found `notModeled[]` is itself INCOMPLETE — the presented heating total omits F280's **mandatory air-change term** (air leakage AIM-2 + ventilation, Cl. 5.2.3; ~20–40% of the load) and duct/pipe loss (Cl. 5.2.4/5.2.5), and neither is listed. So the panel currently presents the number as more complete than it is. The conductive spine (above-grade + BASESIMP ground) CONFORMS; the TOTAL does not. Sequence step 1 (see BUILD_ROADMAP.md Session-80 block) will add air-change/vent/duct/pipe to `notModeled[]` and REMOVE `solar-gain` from it (audit #139 revises the "stays permanently" line just above — solar is correctly cooling-side per #130, but a cooling-only term should not sit in a list of HEATING gaps). Full findings: ADDITIONAL_FUNCTIONALITY.md #132–#140. **UPDATE — sequence step 1 DONE (Session 81, §11e):** an interim block-load infiltration term is now modeled + folded into the total; `notModeled[]` discloses infiltration(partial)/ventilation/duct-pipe precisely and `solar-gain` is removed (#139); ground `Toh`/`Ti` consistency fixed (#134/#135). Ventilation + duct/pipe MODELLING still open.
...
- **Climate reuse:** `station` ("City\|\|\|Region") passed straight through — the engine's own `resolveClimate` reproduces the `toh`/`dgtemp` Collabinator already resolves (verified: 679/679 key-identical weather tables). No second climate path. **(Session-81 update — supersedes this note:** the ground engine now ALSO receives `designHeatingDBT: resolvedConfig.toh` (#134, override-aware) and `roomTempC: tiC` (#135, the `ti-heating` seam, default 22) — it no longer uses a fixed Troom=22 or the raw station dhdbt. See §11e.)
...
## 11e. ACH50 block-load infiltration + shared resolved-temperature seam (Session 81; commit `a49d9ad`)

Executes sequence step 1 of the Session-80 audit plan, and goes past disclosure-only: an interim **whole-building block-load infiltration** term is now MODELED and folded into the F280 heating total; plus #134/#135/#139.

**Airtightness input:** `AIRTIGHTNESS_OPTIONS` (0.6/1.0/1.5/2.5/3.57 ACH50 + `custom` sentinel; default **2.5** — mid-range existing construction). New **'Air Leakage'** CONFIG_FIELDS category: `airtightness-ach50` (select) + `ach50-custom` (`kind:'number'`). **`resolve-ach50`** cross-field rule mirrors `resolve-toh`: custom number wins only when class === `'custom'`; else `Number(class)`; `resolvedConfig.ach50` is DERIVED, null when unset, never stored.

**Air constant:** `AIR_VOL_HEAT_CAPACITY = 1200` J/(m³·K) (≈1.2 kg/m³ × 1005 J/kg·K), module-level near `F280_TI_HEATING`.

**Infiltration inside `deriveF280Heating` (self-contained):** `volumeM3 = footprintAreaM2 × conditionedHeightM`, where footprint = the `slab-surface` `grossAreaM2` and height = `max(ceilingZm over wall-surface elements) − slab.floorZm` — both read from the `enumeration` the function already receives (NO new geometry takeoff). `infiltrationW = volumeM3 × (ach50/3600) × AIR_VOL_HEAT_CAPACITY × deltaT`, RAW ACH50, using the SAME resolved `deltaT` (`tiC−tohC`) as the conductive spine. **Honest-absence:** no slab footprint / no floor heights (null ceilingZm) / no ACH50 → `infiltration: null` (NOT zero), line hidden. `total = above-grade conductive + ground (when ok) + infiltration (when present)`.

**Panel reframe:** per-section figures labeled **"(partial subtotal)"**; new **Infiltration (block load)** zone (volume, footprint×height, ACH50, subtotal, "Conservative block-load estimate (likely high); room-by-room AIM-2 pending."); new combined **"Heating load (partial — ventilation & duct/pipe not yet modeled)"** total row (the panel previously had NO combined total). `__dumpF280` extended to the three-part total + infiltration lines.

**notModeled[] → full-string disclosures**, `solar-gain` REMOVED (#139); precise air-change wording: `infiltration — conservative block-load approximation (raw ACH50); room-by-room AIM-2 not yet modeled`, `ventilation — not modeled`, `duct/pipe loss — not modeled`, `floor-over-unheated — not modeled` (+ below-grade/slab when ground unresolved). Panel drops the `.replace(/-/g,' ')` cosmetic.

**#134 (shared Toh):** the `deriveGroundCoupledLoss` adapter passes `designHeatingDBT: resolvedConfig.toh` (override-aware) into the BASESIMP input; the engine's `resolveClimate` already prefers it over raw station `dhdbt`. **#135 (shared Ti — policy reversal):** `engine.js computeGroundCoupledLoss` now reads `roomTempC = input.roomTempC ?? ROOM_TEMP_C` and uses it in every load-assembly term; the adapter passes `roomTempC: tiC`. The prior "fixed 22, ti-heating does NOT enter" comments were rewritten. Above-grade + ground + infiltration now share ONE resolved `tiC`/`tohC`/`deltaT` (single seam — the dual-path compliant-vs-preferred arc #159 wraps it in a two-set loop).

**Golden harness:** `__verifyFixture` **66/66 PASS** (was 56; +10 checks `inf.a–inf.j`: volume, deltaT, infiltrationW, three-part total, solar-removed, three air-change entries, two honest-absence guards, and #135/#134 engine-output shifts). `gc.j/k/l` updated for the full-string `notModeled`. New `infiltrationCheck` sidecar block. `src/basesimp/acceptance.test.js` **3/3 still green** after #135 (no `roomTempC` passed ⇒ falls back to 22). **Destructive-harness reminder:** the 6 opening-**placement** checks (n/p/q) false-fail on a not-truly-fresh restore; run once per FULL reload.

**Verified (Claude preview, fixture-elevation):** station Vernon + assemblies + ACH50 2.5 → infiltration 2549 W (72.8 m³ × ACH50 2.5 × ΔT 42), three-part total 3169 W (441.4 + 178.8 + 2549.2); clear ACH50 → infiltration absent, total 620.1 W; `toh-override −30` + `ti-heating 24` → ground 178.8 → 205.9 W (both overrides flow through); panel renders 3 subtotals + 4.05 kW combined total + conservative disclosure. Marks audit #132 (partial), #134, #135, #139 ADDRESSED.

**§5 approximations (eyes-open, all bias HIGH):** raw ACH50 as design rate (parked #160); full-stack-height volume includes unconditioned space (parked #162); constant-footprint-across-levels.

**NEXT:** ventilation modelling (#132b) + duct/pipe (#136) + AIM-2 room-allocated leakage (needs #146 room model); dual-path compliant-vs-preferred (#159).
