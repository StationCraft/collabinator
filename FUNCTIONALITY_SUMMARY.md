...
**⚠ Session-80 F280 conformance-audit note (docs-only; describes a gap in the CURRENT code, no code changed this session):** the read-only heating-path audit against the licensed CSA F280:12 found `notModeled[]` is itself INCOMPLETE — the presented heating total omits F280's **mandatory air-change term** (air leakage AIM-2 + ventilation, Cl. 5.2.3; ~20–40% of the load) and duct/pipe loss (Cl. 5.2.4/5.2.5), and neither is listed. So the panel currently presents the number as more complete than it is. The conductive spine (above-grade + BASESIMP ground) CONFORMS; the TOTAL does not. Sequence step 1 (see BUILD_ROADMAP.md Session-80 block) will add air-change/vent/duct/pipe to `notModeled[]` and REMOVE `solar-gain` from it (audit #139 revises the "stays permanently" line just above — solar is correctly cooling-side per #130, but a cooling-only term should not sit in a list of HEATING gaps). Full findings: ADDITIONAL_FUNCTIONALITY.md #132–#140.
