...
export function getEligibleShapes(shapes, pageId) {
  const pageIdxs = shapes.map((s, i) => ({ s, i })).filter(({ s }) => s.pageId === pageId && s.shapeKind !== 'grade-line' && s.shapeKind !== 'window' && s.shapeKind !== 'door' && s.shapeKind !== 'equipment-item').map(({ i }) => i)
