...
- Remaining (NOW/NEXT): **wire BASESIMP into the app** — replace the interim Model-B `deriveGroundCoupledLoss` placeholder in `App.jsx` with `src/basesimp/engine.js` (`computeGroundCoupledLoss`), feeding it the Slice-9 `below-grade-wall` + `slab-surface` enumeration quantities + a real climate station. Then solar gain.
- Follow-up (pending session): extract full `Foundation_Weather` station table into a data file (climate is hard-coded to Winnipeg January in the acceptance test only right now).
...
*Last updated: 2026-07-01 (Slice 10 — BASESIMP ground-coupled engine landed in-tree under `src/basesimp/`, isolated, commit `a9ad4cb`; acceptance 3/3 to the watt; not yet wired — next is replacing the interim Model-B `deriveGroundCoupledLoss` placeholder)*
