...
- Remaining (NOW/NEXT): **wire BASESIMP into the app** — replace the interim Model-B `deriveGroundCoupledLoss` placeholder in `App.jsx` with `src/basesimp/engine.js` (`computeGroundCoupledLoss`), feeding it the Slice-9 `below-grade-wall` + `slab-surface` enumeration quantities + a real climate station (now available via `weather.json` `station` key). Then solar gain.
...
*Last updated: 2026-07-01 (Slice 11 — BASESIMP weather-table extraction (679 stations) + climate-by-station lookup, commit `5b50384`; acceptance 3/3 sourcing climate from the table; slab weather confirmed identical to basement; still isolated — next is wiring BASESIMP into the app to replace the interim Model-B `deriveGroundCoupledLoss` placeholder)*
