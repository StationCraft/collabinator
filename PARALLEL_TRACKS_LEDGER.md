...
- Remaining (NOW/NEXT): ground-coupled loss engine (separate engine, CONSUMES below-grade-wall + slab-surface); solar gain.
- Ground-coupled F280 note: `BasementHLR.xls` / `SlabOnGradeHLR.xls` are standalone supplemental calculators (separate from above-grade). Below-grade + slab GEOMETRY quantities now exist (Session 77) — the loss engine is the next real thermal work and is the first to remove entries from `notModeled[]`. Base-level interim = U·A·ΔT vs a ground temperature; full workbook method is the compliance pass.
