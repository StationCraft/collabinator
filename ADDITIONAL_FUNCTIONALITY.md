...
**Note on `ti-heating`:** **DONE (Session 76; commit `44615f2`).** Added `ti-heating` `kind:'number'` CONFIG_FIELD in the 'Climate' category; `deriveF280Heating`'s `tiC` seam now reads `resolvedConfig['ti-heating']` (NaN-guarded) with `F280_TI_HEATING = 22` retained as the module-level fallback. No cross-field rule needed (raw value passes through `resolveEffectiveConfig` untouched). Browser-verified: Ti=21 → ΔT 45°C; cleared → ΔT 46°C (fallback). This retires the **last hardcoded F280 input** — Ti is now project-configurable.
